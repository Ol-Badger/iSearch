...
namespace iSearch
{
	public static class aSearch
	{
		...
		public static void Search(string InputString, bool ControlKeyDown)
		{
			...
			else
			{
				...
				if (IndexOfSpace < 0) //check for single token input
				{
					 ...
					 */
				    URI = SearchDic.ContainsKey(InputString) ? SearchDic[InputString].Site : SearchDic["gg"].ReplaceDollar(InputString);
				}
				else
				{
					 ...
					 */
				    URI = SearchDic.ContainsKey(key) ? SearchDic[key].ReplaceDollar(InputString.Substring(IndexOfSpace+1)) : SearchDic["gg"].ReplaceDollar(InputString);
				}
            }
            if (URI.Contains(".exe"))
                RunLocal(URI);
	    ...
	    static void RunLocal(string target)
	    {
	        if (target.ToLower().EndsWith(".exe"))
	            Process.Start(target);
	        else
	        {
	            int Index = target.IndexOf(".exe", StringComparison.OrdinalIgnoreCase);
	            string param = target.Substring(Index+5);
	            target = target.Substring(0, Index+4);
